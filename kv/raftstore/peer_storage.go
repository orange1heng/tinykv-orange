...
func (ps *PeerStorage) Append(entries []eraftpb.Entry, raftWB *engine_util.WriteBatch) error {
	...
	for _, ent := range entries {
		// 写入raft日志，编码格式：[0x02+ Raft Region ID + 0x01 + Log Index] + msg
	...
	// ...truncated...currLastIndex...raftDB stabled(prevLastIndex)
	// [truncated, currLastIndex]已经被覆盖修改了，只需要删除[currLastIndex,prevLastIndex]之间的即可
...
// 将Ready的状态持久化到底层存储中，包括待持久化的日志、Raft HardState、Snapshot
func (ps *PeerStorage) SaveReadyState(ready *raft.Ready) (*ApplySnapResult, error) {
	...
	// todo: 1. 通过 raft.isEmptySnap() 方法判断是否存在 Snapshot，如果有，则调用ApplySnapshot() 方法应用；
	...
	//  [0x01 + 0x02 + Raft Region ID + 0x02] + RaftLocalState
